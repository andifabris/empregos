...
function emp5.startPayments()
	...
	if user_id then
		if vRP.tryGetInventoryItem(user_id,config.postmanitem,ammount[source]) then
			...
			TriggerClientEvent("Notify",source,"sucesso","Você entregou <b>x"..ammount[source].." encomendas</b>, recebendo <b>$"..vRP.format(parseInt(price+ammount[source])).." reais</b>.",8000)
			ammount[source] = nil
			return true
		else
